...
const ColumnFilter: FC<ColumnFilterProps> = ({ column }) => {
  ...
  const { sortedUniqueValues, uniqueValuesSize } = useMemo(
    () => {
      ...
      if (column.id === "canton") {
        const filteredUniqueValues = uniqueValues.filter(Boolean)
        return {
          uniqueValuesSize: filteredUniqueValues.length,
          sortedUniqueValues: filteredUniqueValues.sort(),
        }
      }

      return {
        uniqueValuesSize: uniqueValues.length,
        sortedUniqueValues: uniqueValues.sort(),
      }
    },
